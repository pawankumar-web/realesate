...
import { UserCircle, Store, Shield } from 'lucide-react'

const roleConfig = {
  user: { label: 'User', icon: UserCircle, hint: 'Browse & save properties', testEmail: 'user@realesate.com' },
  vendor: { label: 'Vendor', icon: Store, hint: 'List & manage properties', testEmail: 'agent@realesate.com' },
  admin: { label: 'Admin', icon: Shield, hint: 'Manage platform & users', testEmail: 'admin@realesate.com' },
} as const
...
export default function LoginPage() {
  ...
  if (user) {
    const dashboard = user.role === 'admin' ? '/admin' : user.role === 'vendor' ? '/vendor' : '/dashboard'
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md space-y-6">
        ...
        {/* Role selector */}
        <div className="flex items-center gap-2 p-1 rounded-2xl glass-card">
          {(Object.entries(roleConfig) as [Role, typeof config][]).map(([key, cfg]) => {
            const Icon = cfg.icon
            return (
              <button
                key={key}
                onClick={() => setSearchParams({ role: key })}
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full h-11 flex items-center justify-center gap-3 rounded-xl border border-border hover:bg-foreground/5 transition-colors text-sm font-medium"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder={`${config.label} email`} />
          </div>
          <div className="space-y-2">
        ...
        {/* Test credentials */}
        <div className="rounded-xl border border-border/50 bg-foreground/[0.02] p-3 text-center">
          <p className="text-[10px] text-muted-foreground/60 uppercase tracking-wider mb-1">Demo Credentials</p>
          <p className="text-xs text-muted-foreground">
            <span className="font-mono">{config.testEmail}</span>
            {' / '}
